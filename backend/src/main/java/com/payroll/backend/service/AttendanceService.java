...
public class AttendanceService {
 ...
 @Transactional(readOnly=true) public List<AttendanceResponse> list(LocalDate from, LocalDate to, com.payroll.backend.security.UserPrincipal principal){return records.findByOrgCodeAndAttendanceDateBetweenOrderByAttendanceDateDesc(org.orgCode(),from,to).stream().filter(record -> canAccess(principal,record.getEmployee())).map(this::response).toList();}
 ...
 private boolean canAccess(com.payroll.backend.security.UserPrincipal principal, Employee employee){try{employeeAccessService.assertCanAccessEmployee(principal,employee);return true;}catch(ResourceNotFoundException ignored){return false;}}
