    ...
    if (scheduleChanged && next.employeeId && next.date) {
      Promise.all([
        ...
        .then(([attendanceRecords, shifts]) => {
          ...
          setManualRows((current) => current.map((row) => row.id === id && row.employeeId === next.employeeId && row.date === next.date ? { ...row, clockIn, clockOut } : row));
        })
        .catch(() => undefined);
    }
  }
